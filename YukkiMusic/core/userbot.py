...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER(__name__).info(f"𝙉𝙖𝙮𝙧𝙖 Starting Assistant Clients")
        if config.STRING1:
            await self.one.start()
            try:
                await self.one.join_chat("ll_ZIDDI_SHAYAR_ll")
                await self.one.join_chat("AboutshayarAkhawab")
                await self.one.join_chat("ll_A_C_H_ll")
                await self.one.join_chat("l_TERI_MERI_BAATE_l")
            except:
                pass
            assistants.append(1)
            try:
                await self.one.send_message(config.LOG_GROUP_ID, "𝙉𝙖𝙮𝙧𝙖 Assistant Started")
            except:
        ...
        if config.STRING2:
            ...
            assistants.append(2)
            try:
                await self.two.send_message(config.LOG_GROUP_ID, "𝙉𝙖𝙮𝙧𝙖 Assistant Started")
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING3:
            ...
            assistants.append(3)
            try:
                await self.three.send_message(config.LOG_GROUP_ID, "𝙉𝙖𝙮𝙧𝙖 Assistant Started")
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING4:
            ...
            assistants.append(4)
            try:
                await self.four.send_message(config.LOG_GROUP_ID, "𝙉𝙖𝙮𝙧𝙖 Assistant Started")
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING5:
            ...
            assistants.append(5)
            try:
                await self.five.send_message(config.LOG_GROUP_ID, "𝙉𝙖𝙮𝙧𝙖 Assistant Started")
            except:
                LOGGER(__name__).error(
